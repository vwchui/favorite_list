...
- **NEVER** hand-write product data (names, SKUs, prices, images). It comes from `ProductService` — see `utilities.md`.
- New pages go in `src/pages/<PageName>.tsx`; new components in `src/components/<feature-name>/` or `src/components/custom/`. Full decision tree in `living-design-guidelines.md`.
- Set the active theme before rendering anything — see `theming.md`.
...
|-----------|--------------|
| `theming.md` | always — set the theme before the first render |
| `a11y` | always — before writing any UI |
| `living-design-guidelines.md` | choosing a component; deciding where a new file goes |
| `components-index.md` | browsing the whole catalogue by eye |
| `component-communication.md` | anything touching shared state, cart, or header |
| `utilities.md` | product data, media, icons, illustrations |
...
- **Agent rules**: `rules/` (canonical, always visible); mirrored per-tool in `.claude/rules/`
