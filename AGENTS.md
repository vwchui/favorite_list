...
- **NEVER** hand-write product data (names, SKUs, prices, images). It comes from `ProductService` — see `utilities.mdc`.
- New pages go in `src/pages/<PageName>.tsx`; new components in `src/components/<feature-name>/` or `src/components/custom/`. Full decision tree in `living-design-guidelines.mdc`.
- Set the active theme before rendering anything — see `theming.mdc`.
...
|-----------|--------------|
| `theming.mdc` | always — set the theme before the first render |
| `a11y` | always — before writing any UI |
| `living-design-guidelines.mdc` | choosing a component; deciding where a new file goes |
| `components-index.mdc` | browsing the whole catalogue by eye |
| `component-communication.mdc` | anything touching shared state, cart, or header |
| `utilities.mdc` | product data, media, icons, illustrations |
...
- **Agent rules**: `rules/` (canonical, always visible); mirrored per-tool in `.cursor/rules/`
